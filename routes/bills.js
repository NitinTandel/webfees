...
router.get('/', function(req, res, next){
    if (req.isAuthenticated()) {
         ...
         req.flash('apiURLDev', config.apiURLDev);
         req.flash('apiURLProd', config.apiURLProd);
         req.flash('currentEnv', config.currentEnv);
         req.flash('isProdEnv', config.currentEnv == "PROD");
         res.render('makebills', {apiURL: config.currentEnv == "PROD" ? config.apiURLProd : config.apiURLDev , isProdEnv : config.currentEnv == "PROD" })

       /*
        res.render('makebills', {apiURLDev : config.apiURLDev, apiURLProd : config.apiURLProd,
            helpers: {
                isProdEnv() { return config.currentEnv == "PROD"; }
              }   
        });
        */
...
//            db.get("SELECT BILLID, BILL_NO, BILL_DT, (IFNULL(TOT_AMT, 0) - IFNULL(RCPT_AMT,0)) AS BAL_AMT FROM VOUCHERS WHERE AC_CODE= ? AND IFNULL(CLIND, 'N') <> 'Y' AND CO_CODE= ? AND IFNULL(TOT_AMT, 0) - IFNULL(RCPT_AMT,0) > 0 ", [ req.params.PrtId , req.user.DEFAULT_CO], (err, rows) => {
            db.get("SELECT BILLID, BILL_NO, BILL_DT, (IFNULL(TOT_AMT, 0) - IFNULL(RCPT_AMT,0)) AS BAL_AMT FROM VOUCHERS WHERE AC_CODE= ? AND IFNULL(CLIND, 'N') <> 'Y' AND CO_CODE= ? ORDER BY BILLID DESC LIMIT 1", [ req.params.PrtId , req.user.DEFAULT_CO], (err, rows) => {
            if (err) {
                  res.json({"message" : "No Data", "error" : "No"} );
                } else {
                  res.json(rows); 
...
router.post('/billinfo', (req, res, next) => {
  //console.log(req.body)
  const allCompany = req.body.FOR_ALL_COMPANY;
  const allGroup = req.body.FOR_ALL_GROUP;
  const groupList = req.body.GROUPLIST;
  const currentYear = req.user.DEFAULT_YEAR
  const currentCompany = req.user.DEFAULT_CO

//  const currentYear = "2020-21"
//  const currentCompany = 3
  const rtnData = {
    billsList :[] , // Distinct Group Id, Ac_code, Co Id, Bill Id sorted by Grp id and then Ac_code
    billHeaders : [],
    billLines : [] 
  } ;

  const strgrouplist = groupList.length ? ( "'" + groupList.join("', '") + "'" ) : '';
  const grpCriteria = allGroup ? "" : `AND B.GRP_CODE IN ( ${strgrouplist} )`

  const comCriteria = allCompany ? "" : " AND A.CO_CODE = " + currentCompany

  let db = new sqlite3.Database( config.database, sqlite3.OPEN_READWRITE, (err) => {  
    if (err) {
      res.json(err.message);
    }
    const sqlstmt = `SELECT DISTINCT B.GRP_CODE, B.AC_CODE, A.CO_CODE, A.BILLID
    ...
    const sqlstmt1 = `SELECT A.*, B.AC_NAME, B.ADDRESS1, B.ADDRESS2, B.ADDRESS3, B.ADDRESS4, B.PAN1
                    FROM VOUCHERS A, ACC_MAST B  
                    WHERE A.AC_CODE=B.AC_CODE 
                    AND A.YEAR = ? 
                    ${comCriteria} 
    ...
    const sqlstmt2 = `SELECT C.BILLID, C.CO_CODE, A.YEAR, C.BILL_NO, C.BILL_DT, C.TRCD, D.DESC AS TRCDDESC, C.SUBCD, E.DESC AS SUBDESC, C.REMARKS, C.BILL_AMT  
                    FROM VOUCHERS A, ACC_MAST B, TRANSACT C, TRCD D, TRCDSUB E 
                    WHERE A.AC_CODE = B.AC_CODE 
                    AND A.CO_CODE = C.CO_CODE
                    AND A.BILLID = C.BILLID 
                    AND C.TRCD=D.TRCD 
                    AND C.TRCD=E.TRCD AND C.SUBCD = E.SUBCD 
                    AND A.YEAR = ?
                    ${comCriteria} 
                    ${grpCriteria} 
                    ORDER BY A.AC_CODE; `
    ...
    db.all(sqlstmt,[ currentYear ],  (err, rows) => {
      if (err) {
          res.json(err.message);
      } else {                          
        rtnData.billsList = rows;   
        db.all(sqlstmt1,[ currentYear ], (err, rows) => {
          if (err) {
            res.json(err.message);
          } else {
            rtnData.billHeaders = rows; 
            db.all(sqlstmt2,[ currentYear ], (err, rows) => {
              if (err) {
                res.json(err.message);
              } else {
                rtnData.billLines = rows;
                const finalData = []
                for(var i=0; i < rtnData.billHeaders.length ; i++){
                  var filtdata = rtnData.billLines.filter( el => el.CO_CODE == rtnData.billHeaders[i].CO_CODE && el.BILLID == rtnData.billHeaders[i].BILLID )
                  var billdata =  rtnData.billHeaders[i]
                  billdata.transactions = filtdata
                  finalData.push( {data : billdata}  )
                }
            ...
            db.serialize(function() {
              ...
              db.get('SELECT MAX(BILLID)+1 AS NEWBILLID FROM VOUCHERS',  [ ], function(err, row) {
                ...
                db.get('SELECT MAX(CAST(BILL_NO AS LONG ))+1 AS NEWBILL_NO FROM VOUCHERS WHERE CO_CODE = ? AND YEAR = ?',  [ billData.CO_CODE, billYear], function(err, row) {
                  ...
                  // Must get only 1 row of last bill id 
                  db.get("SELECT BILLID, BILL_NO, BILL_DT, (IFNULL(TOT_AMT,0) - IFNULL(RCPT_AMT,0)) AS BAL_AMT FROM VOUCHERS WHERE AC_CODE= ? AND IFNULL(CLIND, 'N') <> 'Y' AND CO_CODE= ? ORDER BY BILLID DESC LIMIT 1 ", [ billData.AC_CODE, billData.CO_CODE], (err, rows) => {
                    if (!rows) {
                      prvBal = 0 ;
                    } else {
                      if (rows.BAL_AMT > 0) {
                    ...
                    var sqlstmt = "INSERT INTO VOUCHERS (BILLID, CO_CODE, BILL_NO, BILL_DT, AC_CODE, BILL_FOR_YEAR, BILL_AMT, TOT_AMT, YEAR, LASTBILLID) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )" ;
                    db.run(sqlstmt, [ billId, billData.CO_CODE, billNo, billData.BILL_DATE, billData.AC_CODE, billData.BILL_FOR_YEAR, BillAmt, TotAmt, billYear, prvBillId ], function(err, row){ 
                      db.serialize(function() {
                        isError =false; 
                        for(var i=0; i< TranData.length; i++){
                          var valTRCD = TranData[i].TRCD ;
                ...
                let db = new sqlite3.Database(config.database, sqlite3.OPEN_READWRITE, (err) => {
                  ...
                  db.get(sqlstmt, [billData.CO_CODE, billid ], function(err, row){ 
                    if (!row) {
                      ...
                      var BillAmt = TotAmt ;
                      var sqlstmt = "UPDATE VOUCHERS SET BILL_FOR_YEAR = ? , BILL_AMT = ? , TOT_AMT = ? WHERE CO_CODE = ? AND BILLID = ?" ;
                        db.run(sqlstmt, [ billData.BILL_FOR_YEAR,  BillAmt, TotAmt, billData.CO_CODE, billid], function(err, row){
                            if (err){ 
                                isError = true;
                                errMsg = err.message ;
                            } else {
...
router.post('/billexport', (req, res, next) => {
  ...
  let db = new sqlite3.Database(config.database, sqlite3.OPEN_READWRITE, (err) => {
    ...
    var sqlstmt = `SELECT A.BILLID, A.CO_CODE, SUBSTR('0000'|| A.BILL_NO, -4, 4) AS BILL_NO , A.BILL_DT
                  , A.AC_CODE, B.AC_NAME , IFNULL(A.BILL_FOR_YEAR, ' ') AS BILL_FOR_YEAR, (C.TRCD || '_' || C.SUBCD) AS TRCD_CODE 
    ...
    db.all(sqlstmt, [coCode , fyear ], function(err, rows){
      if(err) {
        ...
        if(rows.length == 0) {
          ...
          rows.forEach((row) => {                
            k++;
            dataline = row.CO_CODE + "|" + row.BILLID + "|" + row.BILL_NO + "|" + row.BILL_DT + "|" + row.AC_NAME + "|" + row.AC_CODE + "|" + row.BILL_FOR_YEAR + "|" + row.TRCD_CODE + "|" + row.BILL_AMT + endOfLine ; 
            res.write(dataline);
            if(k==rows.length) {
              res.end();
            }
