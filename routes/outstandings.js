  ...
  //if all companies flag  then get list for all companies
	...
	if (err) {
    	res.json(err.message) ; 
   		...
   		db.all(`SELECT C.GRP_CODE, A.AC_CODE, C.AC_NAME, B.COMPANY, A.BILL_NO, A.BILL_DT, A.TOT_AMT
              , IFNULL(A.RCPT_AMT, 0) AS RCPT_AMT , IFNULL(A.TDS_AMT,0) AS TDS_AMT
              ...
              AND B.CO_CODE = ?
              ORDER BY 1, 2`, [ req.user.DEFAULT_CO], (err, rows) => {
        	if (err) {
        		res.json(err.message);
        	} else {
        		res.json(rows);	
...
router.post('/allOS', (req, res, next) => {
//  console.log(req.user.DEFAULT_CO);
  //if all companies flag  then get list for all companies
  var currentCoCode = req.user.DEFAULT_CO
  var currentYear = req.user.DEFAULT_YEAR
  var grplist = req.body.grouplist ; 
  var tillCurrentYr = req.body.tillCurrentYr 

  var sqlstmt = `SELECT C.GRP_CODE, A.AC_CODE, C.AC_NAME, B.COMPANY, A.BILL_NO, A.BILL_DT, A.TOT_AMT
              , IFNULL(A.RCPT_AMT, 0 )  AS RCPT_AMT , IFNULL(A.TDS_AMT,0) AS TDS_AMT
              , CASE WHEN A.CLIND = 'Y' THEN 0 ELSE (IFNULL(A.TOT_AMT,0) - IFNULL(A.RCPT_AMT, 0) - IFNULL(A.TDS_AMT, 0)) END AS BAL_AMT 
              FROM VOUCHERS A, COMPANY B, ACC_MAST C
              WHERE A.CO_CODE = B.CO_CODE
              AND A.AC_CODE=C.AC_CODE
              AND (A.CLIND <> 'Y' OR A.CLIND IS NULL)
              AND (IFNULL(A.TOT_AMT,0) - IFNULL(A.RCPT_AMT, 0) - IFNULL(A.TDS_AMT, 0)) <> 0
              <<YEAROPTION>>
              <<SPECIFICCOMPANYOPTION>> 
              <<SPECIFICGROUPOPTION>> 
              ORDER BY 1, 2` ;

  if (req.body.companyscope == 2 ) {
      sqlstmt = sqlstmt.replace('<<SPECIFICCOMPANYOPTION>>' , 'AND A.CO_CODE = ' + currentCoCode ) ;  
  } else {
      sqlstmt = sqlstmt.replace('<<SPECIFICCOMPANYOPTION>>' , ' '  ) ;  
  }

  if(tillCurrentYr) {
    sqlstmt = sqlstmt.replace('<<YEAROPTION>>' , " AND  YEAR <= '" + currentYear + "' "  ) ;  
  } else {
    sqlstmt = sqlstmt.replace('<<YEAROPTION>>' , ' '  ) ;  
  }


  if (req.body.datascope == 2 ) {
    sqlstmt = sqlstmt.replace('<<SPECIFICGROUPOPTION>>' , 'AND GRP_CODE IN ' + grplist ) ;  
  } else {
    sqlstmt = sqlstmt.replace('<<SPECIFICGROUPOPTION>>' , ' '  ) ;  
  }
            ...
            db.serialize(function() {
              ...
              db.get('SELECT MAX(BILLID)+1 AS NEWBILLID FROM VOUCHERS',  [ ], function(err, row) {
                ...
                db.get('SELECT MAX(CAST(BILL_NO AS LONG ))+1 AS NEWBILL_NO FROM VOUCHERS WHERE CO_CODE = ? AND YEAR = ?',  [ billData.CO_CODE, billYear], function(err, row) {
                  ...
                  db.get("SELECT BILLID, BILL_NO, BILL_DT, (IFNULL(TOT_AMT,0) - IFNULL(RCPT_AMT,0)) AS BAL_AMT FROM VOUCHERS WHERE AC_CODE= ? AND CLIND <> 'Y' AND CO_CODE= ? ", [ billData.AC_CODE, billData.CO_CODE], (err, rows) => {
                    if (!rows) {
                      prvBal = 0 ;
                    } else {
                      if (rows.BAL_AMT > 0) {
